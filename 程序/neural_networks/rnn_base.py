...
class RNNBase(object):
	...
	def __init__(self, 
		...
		self.file_log = open(r"G:\毕业设计\代码\结论\log.txt", "w")
...
#		print(self.predict_function(X, mask.astype(theano.config.floatX)).shape)
	...
	def train(self, dataset, 
		...
		self.file_log.write("iterations\tepochs\ttime")
		ms = []
		for m in self.metrics:
			ms.append(m)
			self.file_log.write("\t{}".format(m))
		self.file_log.write("\n")
		try: 
			while (time() - start_time < max_time and iterations < max_iter):

				#if dataset.training_set.epochs > 50:
					#self.file_log.close()
					#break
...
#				if progress_indicator >= next_save:
				if progress_indicator >= next_save or time()-start_time>3600:

					if progress_indicator >= min_iterations or time()-start_time>3600:
						...
						# Print info
						self._print_progress(iterations, epochs[-1], start_time, train_costs, metrics, validation_metrics, ms = ms)
...
#						next_save += min(max_progress_interval, next_save * (progress - 1))
					next_save += 10000 if next_save>10000 else next_save
	...
	def _print_progress(self, iterations, epochs, start_time, train_costs, metrics, validation_metrics, ms=[]):
		...
		self.file_log.write("{}\t{}\t{}".format(iterations, epochs, time() - start_time))
		for m in ms:
			self.file_log.write("\t{}".format(metrics[m][-1]))
		self.file_log.write("\n")

		# Print on stderr for easier recording of progress
		print(iterations, epochs, time() - start_time, train_costs[-1], ' '.join(list(map(str, [metrics[m][-1] for m in self.metrics]))), file=sys.stderr)
