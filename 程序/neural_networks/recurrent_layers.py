...
def get_recurrent_layers(args):
	return RecurrentLayers(layer_type=args.recurrent_layer_type, layers=list(map(int, args.r_l.split('-'))), bidirectional=args.r_bi, embedding_size=args.r_emb)
...
class RecurrentLayers(object):
	...
	def set_name(self):
		...
		self.name += "h"+('-'.join(list(map(str,self.layers))))
